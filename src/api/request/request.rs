...
impl PromisseSankhya {
    ...
    pub async fn save_ped<C: Serialize + Debug, T: DeserializeOwned + Debug>(
        &self,
        jsons: Vec<C>,
    ) -> Result<Vec<(i32, T)>, Box<reqwest::Error>> {
        let pb = SpinnerBuilder::new("Inserting...");
        ...
        let last_char = &self.url.chars().last().unwrap();
        let endpoint = "mgecom/service.sbr?serviceName=CACSP.incluirNota&outputType=json&mgeSession=";
